...
import { COLORS, GEOMETRY_CONFIG } from '../constants';
...
const CommutatorSegment: React.FC<{ rotationZ: number }> = ({ rotationZ }) => {
...
const RectCoil: React.FC = () => {
  const { radius: r, width: w, length: l } = GEOMETRY_CONFIG.coil;
  const mat = <meshStandardMaterial color={COLORS.coil} metalness={0.6} roughness={0.3} />;
...
export const Rotor: React.FC<RotorProps> = ({ speed }) => {
